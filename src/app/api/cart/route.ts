...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.productId) {
      return NextResponse.json({ message: "ID du produit requis." }, { status: 400 })
    }
    ...
    if (isNaN(utilisateurIdInt) || isNaN(productIdInt)) {
      return NextResponse.json({ message: "ID utilisateur et produit doivent être valides." }, { status: 400 })
    }
    ...
    const imageFinale = product.image || "file.svg"
